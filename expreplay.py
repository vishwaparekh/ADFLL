...
class ReplayMemory(object):
    def __init__(self, max_size, state_shape, history_len,agents):
        ...
        self.weights=np.ones((self.agents,self.max_size),dtype='int32')
    def append(self, exp):
...
class ExpReplay(DataFlow, Callback):
    ...
    def __init__(self,
                 ...
                 init_exploration,
                 update_frequency, history_len,agents=2,prev_exp=None):
        """
        Args:
    ...
    def _populate_exp(self):
        ...
        for i in range(0,self.agents):
            if isOver[i]:
                ...
                self._player_distError[i].feed(info['distErrorMM_{}'.format(i)])
            if np.all(isOver):
                self.player.reset()
    ...
    def get_data(self):
        ...
        while True:
            ...
            if self.prev_exp is not None:
                #print('Sampling previous experience')
                #print(len(batch_exp))
                for pe in self.prev_exp:
                    idx = self.rng.randint(
                        self._populate_job_queue.maxsize * self.update_frequency,
                        len(pe) - self.history_len - 1,
                        size=(int(self.batch_size/len(self.prev_exp))+1))
                    for i in idx:
                        batch_exp.append(pe.sample(i))
                    #print(len(batch_exp))
