...
class filesListCardioMRLandmark(object):
    ...
    def __init__(self, files_list=None, returnLandmarks=True,agents=2):
        ...
        for line in open(files_list[0]):
            line = line.strip()
            if line: self.image_files.append(line)
        ...
        if self.returnLandmarks:
            ...
            for line in open(files_list[1]):
                line = line.strip()
                if line: self.landmark_files.append(line)
            ...
            for idx in indexes:
                ...
                if self.returnLandmarks:
                    ...
                    for i in range(0,self.agents):
                        ...
                        # landmarks.append(np.round(all_landmarks[(i+4)% 6]).astype('int'))# Apex + MV
                        # landmarks.append(np.round(all_landmarks[(i+3)% 6]).astype('int')) #LV + Apex
                        # landmarks.append(np.round(all_landmarks[((i + 1) + 3) % 6]).astype('int'))  # LV + MV
                else:
                    landmarks = None
    ...
    def sample_circular(self,shuffle=False):
        ...
        while True:
            for idx in indexes:
                ...
                image_filenames=[]
                for i in range(0,self.agents):
                    # landmarks.append(np.round(all_landmarks[(i*2+10)%13]).astype('int')) ### 10 RC ; 11 LC ; 12 CSP
                    landmarks.append(np.round(all_landmarks[(i+10) % 13]).astype('int'))
                    images.append(image)
                    image_filenames.append(self.images_list[idx][:])
                yield images, landmarks, image_filenames, sitk_image.GetSpacing()
