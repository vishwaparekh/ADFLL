...
class MedicalPlayer(gym.Env):
    ...
    def __init__(self, directory=None, viz=False, task=False, files_list=None,
        ...
        if directory is not None:
            ...
            self.log_dir = os.path.join(dir_path, 'logs')
        self.reset_logs()
    ...
    def step(self, act, q_values, isOver):
        ...
        self.dist_pixel = np.zeros((self.agents,))
        ...
        for i in range(0, self.agents):
            ...
            info['distErrorPix_{}'.format(i)] = self._location[i]
            self.dist_pixel[i]=dist_error_pixels
            info['distErrorMM_{}'.format(i)] = self._target_loc[i]
            # overlap in 3D 
            if self.xscale % 2:
    ...
    def get_agents_terminal_landmarks_data(self):
        '''
        Once the agents finish with an episode, statistics are gathered from the terminal state (final
        prediction) of the agents. Statistics include:

            (1) Landmark prediction coordinates.
            (2) Euclidean distance between prediction and ground truth in the xy plane.
            (3) Euclidean distance between prediction and ground truth along the z axis.
            (4) Euclidean distance between prediction and ground truth.

        :return: A list with an entry for each agent, which contains statistics about the agent's prediction.
        '''

        x_scaling, y_scaling, z_scaling = self.spacing[0], self.spacing[1], self.spacing[2]
        agents_terminal_landmarks_data = []
        for agent_loc_history, target_loc, trec, rec in zip(self._location, self._target_loc,self.target_rectangle,self.rectangle):
            terminal_landmark = agent_loc_history

            x_mm = terminal_landmark[0] #* x_scaling
            y_mm = terminal_landmark[1] #* y_scaling
            z_mm = terminal_landmark[2] #* z_scaling

            xt_mm = target_loc[0] #* x_scaling
            yt_mm = target_loc[1] #* y_scaling
            zt_mm = target_loc[2] #* z_scaling

            xy_dist = np.sqrt((x_mm - xt_mm)**2 + (y_mm - yt_mm)**2)#np.sqrt((x_mm - xt_mm)**2 + (y_mm - yt_mm)**2)
            
            dist = np.linalg.norm(target_loc - terminal_landmark)#np.sqrt((x_mm - xt_mm)**2 + (y_mm - yt_mm)**2 + (z_mm - zt_mm)**2) #self.iou[0]



            intersection = volume(trec, rec)
            volume_rectangle = volume(rec, rec)
            volume_target_rectangle = volume(trec, trec)
            union = volume_rectangle + volume_target_rectangle - intersection
            IoU = intersection / union


            agents_terminal_landmarks_data.append((terminal_landmark, xy_dist, IoU, dist))
        return agents_terminal_landmarks_data

    def log_terminal_landmark_results(self):
        '''
        Once the agents finishes an episode, obtain statistics about the predictions and log them to a file.
        '''

        # Directory of this python file
        log_path = os.path.join(self.log_dir, 'terminal_landmarks.txt')
        f = open(log_path, "a")
        f.write(self.filename[0] + '\n')
        terminal_landmarks_data = self.get_agents_terminal_landmarks_data()
        for (x, y, z), xy_dist, z_dist, dist in terminal_landmarks_data:
            f.write(f'Loc ({x}, {y}, {z}) px, XY Dist {xy_dist} pixel, IOU {z_dist} %, Dist {dist} pixel\n')
        f.close()

    def reset_logs(self):
        '''
        Creates folder for logs (if necessary). Removes existing file to store end of episode statistics.
        Creates folder to store agent predictions (gifs).
        '''

        if not os.path.isdir(self.log_dir):
            os.makedirs(self.log_dir)

        terminal_landmark_path = os.path.join(self.log_dir, 'terminal_landmarks.txt')
        if os.path.isfile(terminal_landmark_path):
            os.remove(terminal_landmark_path)

        agent_images_path = os.path.join(self.log_dir, 'agent_images')
        if not os.path.isdir(agent_images_path):
            os.makedirs(agent_images_path)
    ...
    def display(self, return_rgb_array=False):
        if all(self.terminal):
            '''
            Save best landmarks at the end of each episode
            '''
            self.log_terminal_landmark_results()
        #to display images during eval, change this to False
        if True:
           return 
        ...
        target_points = []
        current_points = []
        max_cols = 5  # Max four images per row
        max_cols = max_cols if self.agents > max_cols else self.agents
        rows = []
        ...
        for i in range(0, self.agents):
            ...
            if i != 0:
                '''
                The resulting gif will be a collage of results from each agent; the shifts_x and shifts_y offsets
                here are used to identify where each frame is for each agent given the collaged image. 
                '''
                assert shape == current_plane.shape
                if i % max_cols == 0:
                    # Need to start new row
                    rows.append(planes)
                    planes = current_plane
                    shifts_x.append(0)
                    shifts_y.append(shape[0] + shifts_y[-1])
                else:
                    # Continue collage from the next column 
                    planes = np.hstack((planes, current_plane))
                    shifts_x.append(shape[1] + shifts_x[-1])
                    shifts_y.append(shifts_y[-1])

        if self.agents % max_cols != 0:
            # The last row is not completely filled
            # Need to pad the last row before appending to rows
            num_filler_imgs = max_cols - self.agents % max_cols
            a_filler_img = np.zeros(shape, dtype=np.float32)
            filler_imgs = np.tile(a_filler_img, (1, num_filler_imgs))
            planes = np.hstack((planes, filler_imgs))

        rows.append(planes)
        planes = np.vstack(rows)
        ...
        if self.saveGif:
            ...
            if all(self.terminal):
                ...
                gifname = os.path.join(self.log_dir, 'agent_images', gifname)
                gifname_terminal = self.filename[0].split('.')[0] + '_{}_terminal.gif'.format(i)
                agent_name=self.log_dir.split('Agent')[-1][0]
                if agent_name=='M':
                    agent_name+='(Lifelong Learning)'
                elif agent_name=="X":
                    agent_name+='(Best Baseline)'
                else:
                    agent_name+='(SHELL)'
                gifname='Agent'+agent_name+'_'+'_'.join(gifname.split('_')[-4:])
                gifname_terminal='Agent'+agent_name+'_'+'_'.join(gifname_terminal.split('_')[-4:])
                
                
                gifname = os.path.join(self.log_dir, 'agent_images', gifname)
                gifname_terminal = os.path.join(self.log_dir, 'agent_images', gifname_terminal)
